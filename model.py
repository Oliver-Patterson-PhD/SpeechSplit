...
class LinearNorm(torch.nn.Module):
    def __init__(self, in_dim, out_dim, bias=True, w_init_gain='linear'):
        ...
        torch.nn.init.xavier_uniform(self.linear_layer.weight, gain=torch.nn.init.calculate_gain(w_init_gain))
    ...
    def __init__(self, hparams):
        ...
        for i in range(1):
            conv_layer = torch.nn.Sequential(
                ConvNorm(self.dim_freq if i == 0 else self.dim_enc_2, self.dim_enc_2, kernel_size=5, stride=1, padding=2, dilation=1, w_init_gain='relu'),
                torch.nn.GroupNorm(self.dim_enc_2 // self.chs_grp, self.dim_enc_2))
            convolutions.append(conv_layer)
        self.convolutions = torch.nn.ModuleList(convolutions)
        self.lstm = torch.nn.LSTM(self.dim_enc_2, self.dim_neck_2, 1, batch_first=True, bidirectional=True)
...
class Encoder_6(torch.nn.Module):
    ...
    def __init__(self, hparams):
        ...
        for i in range(3):
            conv_layer = torch.nn.Sequential(
                ConvNorm(self.dim_f0 if i == 0 else self.dim_enc_3, self.dim_enc_3, kernel_size=5, stride=1, padding=2, dilation=1, w_init_gain='relu'),
                torch.nn.GroupNorm(self.dim_enc_3 // self.chs_grp, self.dim_enc_3))
            convolutions.append(conv_layer)
        self.convolutions = torch.nn.ModuleList(convolutions)
        self.lstm = torch.nn.LSTM(self.dim_enc_3, self.dim_neck_3, 1, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        codes = torch.cat((out_forward[:, self.freq_3 - 1::self.freq_3, :], out_backward[:, ::self.freq_3, :]), dim=-1)
        return codes
...
class Encoder_7(torch.nn.Module):
    ...
    def __init__(self, hparams):
        ...
        for i in range(3):
            conv_layer = torch.nn.Sequential(
                ConvNorm(self.dim_freq if i == 0 else self.dim_enc, self.dim_enc, kernel_size=5, stride=1, padding=2, dilation=1, w_init_gain='relu'),
                torch.nn.GroupNorm(self.dim_enc // self.chs_grp, self.dim_enc))
            convolutions.append(conv_layer)
        self.convolutions_1 = torch.nn.ModuleList(convolutions)
        self.lstm_1 = torch.nn.LSTM(self.dim_enc, self.dim_neck, 2, batch_first=True, bidirectional=True)
        # convolutions for f0
        convolutions = []
        for i in range(3):
            conv_layer = torch.nn.Sequential(
        ...
        self.lstm_2 = torch.nn.LSTM(self.dim_enc_3, self.dim_neck_3, 1, batch_first=True, bidirectional=True)
        self.interp = InterpLnr(hparams)
...
class Decoder_3(torch.nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.dim_freq = hparams.dim_freq
        self.dim_neck_3 = hparams.dim_neck_3
        self.lstm = torch.nn.LSTM(self.dim_neck * 2 + self.dim_neck_2 * 2 + self.dim_neck_3 * 2 + self.dim_emb, 512, 3, batch_first=True, bidirectional=True)
...
class Decoder_4(torch.nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.dim_f0 = hparams.dim_f0
        self.dim_neck_3 = hparams.dim_neck_3
        self.lstm = torch.nn.LSTM(self.dim_neck_2 * 2 + self.dim_neck_3 * 2, 256, 2, batch_first=True, bidirectional=True)
